...
// подключение без кода доступа // ?????????????? переписать комментарий
...
function OnOffSecurityMode(val) {
	//openTermForm();
	Sound('click');
	let dataToSend = new String('>'); 		// (0x40, 0x3E) - включить режим Охраны
	if (val) {
		...
		switch (checkedBoxes) {
			case 'Включить контроль датчиков':
				dataToSend += 'Z';      // (0x5A)
				break;
			case 'Включить режим SOS':
				dataToSend += 'E';      //(0x45)
				break;
			case 'Отправить сообщение':
				      // здесь прописать задачу для PWA
				break;
			default:
				break;
		}
		...
		securityMode = true;
		StandBy = false;
		document.getElementById("btn_On_Security_Mode").style.display = "none";
		document.getElementById("btn_Off_Security_Mode").style.display = "block";
	} else {
		sendOut('<');		//(0x3C) - выключить режим Охраны
		securityMode = false;
		StandBy = true;
		document.getElementById("btn_Off_Security_Mode").style.display = "none";
		document.getElementById("btn_On_Security_Mode").style.display = "block";
	}	
	dataToSend = '';
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function openForm(needForm) {
	...
	if (needForm == 'form-security-mode') {
		if (securityMode) {
			OnOffSecurityMode(true);
		} else OnOffSecurityMode(false);
	}

	if (needForm == 'form-menu-stair') {
	//		sendOut('@o');		// (0x40, 0x6F)  принудительно переводим контроллер в режим ожидания
		if (!securityMode) {			// если не режим Охраны
	...
	if (needForm == 'form-scenario-set') {
		sendOut('!'+'10'); 		// запрос значения яркости от устройства (0x21, 10)
		if(connectionFlagWiFi){
			setTimeout(getBrightValue,1000);
		} else setTimeout(getBrightValue,300);
	}
	
	if (needForm == 'form-non-code-connect') {
		sendOut('?'+'100'); 		// запрос всех флагов 
		setTimeout(() => { 
			flagsParser();	
...
document.querySelectorAll('.label').forEach(e => {		// для каждого сценария
   ...
   e.addEventListener('touchstart', function(event) {		// прикосновение тача
		...
		cloneElement.style.left = rect.left + 'px';		//	передаем эти координаты X клону
		cloneElement.style.top = rect.top + 'px';		//	передаем эти координаты Y клону
...
function showVideoBackground(show) {
    const videoWrapper = document.getElementById('bgVideoWrapper');
    ...
    if (show) {
        ...
        // Убираем статичный фон с body
        document.body.style.backgroundImage = 'none';
    } else {
        // Скрываем видео
        if (videoWrapper) videoWrapper.style.display = 'none';
        if (video) video.pause();
        // Восстанавливаем сохраненный фон пользователя
        let savedFone = localStorage.getItem('FONE');
        if (savedFone && savedFone !== 'null') {
            document.body.style.backgroundImage = savedFone;
        } else {
