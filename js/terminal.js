...
function openTermForm(val) {
	...
	} else {		// усли 0, то закрываем форму
...
// массив обрабатываемых событий для окна терминала //
let terminal_event_list = ["mousedown", "touchstart"];

// отслеживаем события из массива (нажатия) для окна терминала //
terminal_event_list.forEach(function(event){
...
// массив обрабатываемых событий для body //
let body_event_list = ["mousemove", "touchmove"];

// отслеживаем события из массива (перемещения) для body //
body_event_list.forEach(function(event){
...
document.body.addEventListener('mouseup', () => {		// для тача тоже работает
  dragging = false
})
