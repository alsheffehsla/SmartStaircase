...
boolean StandbyMode = 0;              // флаг перехода в режим ожидания (выключение работы лестницы, ожидание переключения в боевой режим по Bluetooth)
...
void loop() {
  ...
  if (DataSerialAvailable()) {                         // проверяем, если флаг установлен
    ...
    DataParse();                                         // парсим входные данные
      ...
      if (!StandbyMode && DayOrNight()) {                       // если Ночь и не режим ожидания
        emergencyLighting();                                    // включаем дежурную подсветку крайних ступеней
        top = sensorsCheck(TopSensor, data_Array[7]);           // опрашиваем верхний датчик
        bottom = sensorsCheck(BottomSensor, data_Array[7]);     // опрашиваем нижний датчик
  ...
  do {                                                        // выполнить
    if (data.indexOf(";", 0) != -1) {                             // если блок содержит хоть один разделитель ';' 
      ...
      //  EEPROM.put((addr)*2,val);             временно отключен - переписать запись по требованию приложения     // пишем значение по указанному адресу в EEPROM
        if ((addr = 10)) onlineBright(val);                               // если адрес = 10, то вызов функции немедленного изменения яркости
      }
    } else {                                                      
...
void scenarioParser (String data){                                     // получаем данные
  ...
  do {                                                     
    ...
    cycle_flag = false;                                     // отмена зацикливания сценариев
    ...
    switch (tempdata.charAt(0)) {                              // ищем совпадение по символу
      case 0x4F:                                                    // включение контроллера лестницы
        StandbyMode = false;                                          // выход из режима ожидания
        break;
      case 0x6F:                                                    // выключение контроллера лестницы
        StandbyMode = true;                                           // переход в режим ожидания
        break;
      case 0x50:                                                    // команда ?????????????????????????????
    ...
    if (cycle_flag && data == "") {                       // если установлен флаг зацикливания и команды закончились
  ...
  if (currentTime - startTime >= 1000UL) {          // если разница между временем старта и текущим временем более или равна 1 сек.
    ...
    switch (DarkOrLight()) {                           // запускаем проверку светло/темно 
      ...
      if (darkness == -5) {       // если счетчик темноты достиг -5
        darkness = 0;               // обнуляем счетчик темноты
        night = true;               // устанавливаем ночь
      }
      if (darkness == 5) {        // если счетчик темноты достиг 5
        darkness = 0;               // обнуляем счетчик темноты
        night = false;              // устанавливаем день
      } 
  }
  return night;
 }
